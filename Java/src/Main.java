...
//                new Test(2, SchedulingAlgorithm.ROUND_ROBIN, 15, "RR_Test3", 10),
    ...
    private static int runTest(Test test) {
        ...
        File[] dir = new File("src\\in" + File.separator + test.inFolder).listFiles();
        if (dir != null) {
            for (File input : dir) {
        ...
        try {
            String output = new Scanner(new File("src\\in" + File.separator + test.inFolder + File.separator + "out")).useDelimiter("\\Z").next();
